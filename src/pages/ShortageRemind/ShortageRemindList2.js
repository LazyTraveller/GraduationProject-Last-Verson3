...
import getRemindColumns from './ShortageRemindTable2'
...
class ShortageRemindList extends Component{
  ...
  render() {
    ...
    //     count++;
    //   };
     if (_.get(it, 'remind') === '缺货提醒' && _.get(it, 'status') === true) {
    ...
    return (
      // <PageHeaderWrapper title="" content={content} loading={isSaving} extraContent=''>
        <Card bordered={false} title={<span><Icon type="unordered-list" />已完成补货宿舍列表---总数{remindlist1.length}条</span>} extra={extra}>
          <Row gutter={16}>
            {/* <Col md={10} sm={24}>
