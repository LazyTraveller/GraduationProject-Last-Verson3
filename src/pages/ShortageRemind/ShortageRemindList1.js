...
import getRemindColumns from './ShortageRemindTable1'
...
class ShortageRemindList extends Component{
  ...
  render() {
    ...
    remindlist.map(it => {
      ...
      if (_.get(it, 'remind') === '零食推荐' && _.get(it, 'status') === false) {
    ...
    //   <PageHeaderWrapper title="" content={content} loading={isSaving} extraContent=''>
        <Card bordered={false} title={<span><Icon type="unordered-list" />零食推荐列表</span>} extra={extra}>
          <Row gutter={16}>
            <Col md={10} sm={24}>
